...
public class AllNodesDistance_K_inBinaryTree {
    ...
    public static void main(String[] args) {
        ...
        LogUtil.logIt("Nodes at kth distance " + distanceKNew(root, root.left, 2), true);
...
//        LogUtil.logIt("Nodes at kth distance " + distanceK(root, root, 2), true);
    ...
    public static List<Integer> distanceKNew(TreeNode root, TreeNode target, int k) {
        if (root == null) return new ArrayList<>();

        // Prepare ParentMap
        Map<TreeNode, TreeNode> parentMap = new HashMap<>();
        inorder(root, null, parentMap);

        List<Integer> distanceKNodes = new ArrayList<>();

        Queue<TreeNode> queue = new LinkedList<>();
        queue.add(target);
        queue.add(null); // This is to introduce a new level

        Set<TreeNode> visited = new HashSet<>();

        while (!queue.isEmpty()) {
            TreeNode temp = queue.poll();
            System.out.println("Polled....." + (temp == null ? null : temp.val));
            if (temp == null) {
                k--;
                if (k < 0) {
                    break;
                ...
                if (parentMap.containsKey(temp) && parentMap.get(temp) != null && !visited.contains(parentMap.get(temp))) {
    ...
    private static void inorder(TreeNode root, TreeNode parent, Map<TreeNode, TreeNode> parentMap) {
        if (root == null) return;
        parentMap.put(root, parent);
        inorder(root.left, root, parentMap);
        inorder(root.right, root, parentMap);
    }
